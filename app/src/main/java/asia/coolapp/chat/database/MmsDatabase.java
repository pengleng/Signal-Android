...
import asia.coolapp.chat.attachments.MmsNotificationAttachment;
import asia.coolapp.chat.contactshare.Contact;
import asia.coolapp.chat.database.documents.IdentityKeyMismatch;
import asia.coolapp.chat.database.documents.IdentityKeyMismatchSet;
import asia.coolapp.chat.database.documents.NetworkFailure;
import asia.coolapp.chat.database.documents.NetworkFailureSet;
...
import asia.coolapp.chat.database.model.NotificationMmsMessageRecord;
...
import asia.coolapp.chat.mms.OutgoingGroupUpdateMessage;
...
import asia.coolapp.chat.revealable.ViewOnceExpirationInfo;
...
import static asia.coolapp.chat.contactshare.Contact.Avatar;
...
public class MmsDatabase extends MessageDatabase {
  ...
  public void markIncomingNotificationReceived(long threadId) {
    ...
    if (asia.coolapp.chat.util.Util.isDefaultSmsProvider(context)) {
      SignalDatabase.threads().incrementUnread(threadId, 1);
    }
  ...
  public static class Reader implements MessageDatabase.Reader {
    ...
    private NotificationMmsMessageRecord getNotificationMmsMessageRecord(Cursor cursor) {
      ...
      if (!TextUtils.isEmpty(contentLocation))
        contentLocationBytes = asia.coolapp.chat.util.Util.toIsoBytes(contentLocation);

      if (!TextUtils.isEmpty(transactionId))
        transactionIdBytes = asia.coolapp.chat.util.Util.toIsoBytes(transactionId);
